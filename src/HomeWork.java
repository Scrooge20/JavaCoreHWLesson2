...
//      Второй вариант решения для записи цикла
    ...
    // Для усложнения задачи нельзя пользоваться вспомогательными массивами.
        ...
        int d7 = -2; // на сколько единиц будем смещать массив
        // проверяем переменну смещения на ОТРИЦАТЕЛЬНОЕ или ПОЛОЖИТЕЛЬНОЕ
        // При таком решении задачи нет разницы вправо или лево смещать массив при отрицательном или положительном значении.
        //как назначим - туда и будет смещать
        if (d7 > 0) {
            System.out.println("Сдвиг элементов влево");
            shiftLeft(array7, d7);
        }else{
            int positiveD7 = java.lang.Math.abs(d7);// первое что пришло в голову, что делать с отрицательным числом
            System.out.println("Сдвиг элементов вправо");
            shiftRight(array7, positiveD7);
        }
...
//        rborder = rborder + array6[n];
    ...
    // Задание 7/7. Метод для смещения элементов ВЛЕВО
    public static void shiftLeft (int [] inArray7, int d7){
        for (int i = 0; i < d7 ; i++) { // данный цикл используем для смещения элементов массива. Один проход - одно смещение
            int lostvalue = inArray7[0]; // запоминаем ПЕРВЫЙ элемент, так как смещаем влево
            for (int j = 0; j < Array.getLength(inArray7) - 1 ; j++) { // данный цикл для прохода по элементам массива
                inArray7[j] = inArray7[j + 1];  // присваиваем ЛЕВОМУ элементу значение ПРАВОГО
            ...
            inArray7[inArray7.length - 1] = lostvalue;
        }
        System.out.println("Смещение влево произведено на " + d7 + " значения");
        printformatIntArray(inArray7);
    }
    // Задание 7/7. Метод для смещения элементов ВПРАВО
    public static void shiftRight (int [] inArray7, int d7){
        printformatIntArray(inArray7);
        for (int i = 0; i < d7 ; i++) { // данный цикл используем для смещения элементов массива. Один проход - одно смещение
            int lostvalue = inArray7[Array.getLength(inArray7) - 1]; // запоминаем ПОСЛЕДНИЙ элемент, так как смещаем ВПРАВО

            for (int j = Array.getLength(inArray7) - 1; j > 0 ; j--) { // данный цикл для прохода по элементам массива
                // стартуем осчёт от последнего элемента, и двигаемся к первому элементу. В начало
                inArray7[j] = inArray7[j - 1]; // присваиваем ПРАВОМУ элементу значение ЛЕВОГО
                // используем общепринятую практику - для элемента который может выпасть создадим переменную
                // и в эту переменную запишем "выпадающий элемент" вначале цикла и потом присвоим свободной ячейке
            }
            inArray7[0] = lostvalue;
        }
        System.out.println("Смещение вправо произведено на " + d7 + " значения");
        printformatIntArray(inArray7);
    }
}
